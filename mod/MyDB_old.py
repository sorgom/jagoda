...
class MyDB(MySQL):
    ...
    def reduceObjRecs(self):
        uid = self.getUid()
        if uid:
            recs = self.getFirstCol(f'select TST from USR_OBJ where USR = {uid} limit {USR_RECORDS};')
            if len(recs) == USR_RECORDS:
                self.call(f'delete from USR_OBJ where USR = {uid} and TST < "{recs[-1]}";')

    def recEnt(self, id:int):
        uid = self.getUid()
        if uid:
            self.call(f'replace into USR_OBJ(USR, ENT, TST) values ({uid}, {id}, CURRENT_TIMESTAMP);')
    ...
    def addObjImg(self, objId:int, imgId:int):
        self.call(f'call addObjImg({objId}, {imgId});')
        self.touchEnt(objId)
    ...
    def getObjImgs(self, objId:int):
        return self.getDict(f'select IMG as ID, ORD, imgFileMini(IMG) as SRC from OBJ_IMG where ENT = {objId} order by ORD;')
    ...
    def testData(self):
        ...
        self.multi('USR_OBJ(OBJ, UID, TST)', [f'({id + offset}, {userIdTest}, CURRENT_TIMESTAMP - INTERVAL {n} MINUTE)' for n, id in enumerate(ids)] )
