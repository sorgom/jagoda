...
class MyDB(object):
    def __init__(self, getUidFunc, config):
        ...
        self.config = config
        self.cnx = None # mysql.connector.connect(autocommit=True, **self.config)

    def __del__(self):
        self.cnx.close()

    def cursor(self, **conf):
        if self.cnx is None:
            self.cnx = mysql.connector.connect(autocommit=True, **self.config)
        return self.cnx.cursor(**conf)
    
    def procCursor(self, func, sql:str, args, **conf):
        debug(sql, *args)
        res = None
        cur = self.cursor(**conf)
        try:
    ...
    def get(self, sql:str, *args):
        return self.procCursor(lambda c : c.fetchall(), sql, args)

    def getDict(self, sql:str, *args):
        return self.procCursor(lambda c : c.fetchall(), sql, args, dictionary=True)

    def getOneRow(self, sql:str, *args):
        return self.procCursor(lambda c : c.fetchone(), sql, args)

    def getOneDict(self, sql:str, *args):
        return self.procCursor(lambda c : c.fetchone(), sql, args, dictionary=True)
    ...
    def setWhat(self, objId:int, wId:int):
        self.call('update ART set WHAT = %s where OBJ = %s', objId, wId)
        self.touchObj(objId)
...
def setDB(getUidFunc, config):
    global __mydb__
    if not __mydb__: __mydb__ = MyDB(getUidFunc, config)
    return __mydb__
