...
BEGIN
    declare vOK TINYINT;
    select STDABLE from TTL_STD where ID = pID into @vOK;
    if @vOK THEN
...
BEGIN
    IF pLABEL = '' THEN
        DELETE FROM TTL_ELEM WHERE ID = pID AND ILC = pILC;
    ELSE
        REPLACE INTO TTL_ELEM VALUES (pID, pILC, pLABEL);
    END IF;
END :)  
-- ============================================================
-- objects
